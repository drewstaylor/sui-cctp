...
module message_transmitter_v2::vector_utils {
  // === Errors ===
  ...
  public fun slice<T: copy>(data: &vector<T>, start_index: u64, end_index: u64): vector<T> {
    ...
    if (start_index == end_index && start_index <= data.length()) return result;
  ...
  fun test_slice_normal_case_successful() {
    let data: vector<u64> = vector[1, 2, 3, 4, 5];
  ...
  #[test]
  fun test_slice_start_eq_end_returns_empty() {
      let data: vector<u64> = vector[1, 2, 3, 4, 5];
      assert!(vector::is_empty(&slice(&data, 0, 0)), 0);
      assert!(vector::is_empty(&slice(&data, 5, 5)), 0);
  }
