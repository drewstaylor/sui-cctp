...
module message_transmitter_v2::initialize {
  // === Imports ===
  use message_transmitter_v2::state::{Self};
  use message_transmitter_v2::attester_manager::{Self};
  ...
  public fun init_state(init_cap: InitCap, local_domain: u32, message_version: u32, max_message_body_size: u64, attester: address, ctx: &mut TxContext) {
    ...
    // Reserve nonce 0 so it can never be a valid received nonce.
    state.mark_nonce_used(0);
  ...
  #[test_only] use message_transmitter_v2::state::{State};
  ...
  fun test_init_state_successful() {
    ...
    scenario.next_tx(@0x5);
    {
      ...
      // Nonce 0 is reserved (marked used) at initialization.
      assert!(shared_state.is_nonce_used(0), 3);
