...
module message_transmitter_v2::attester_manager {
    // === Imports ===
    use sui::event::emit;
    ...
    /// Called by the attester manager to enable a new attester.
    /// Requires that the new attester is nonzero and currently not enabled.
    public(package) fun enable_attester_internal(new_attester: address, state: &mut State, ctx: &TxContext) {
      assert_object_version_is_compatible_with_package(state.compatible_versions());
      verify_attester_manager(state, ctx);
      assert!(new_attester != @0x0, EInvalidAttester);
    ...
    // === Test Functions ===
    #[test_only] use sui::{event::{num_events}, test_scenario};
    ...
    #[test]
    #[expected_failure(abort_code = EInvalidAttester)]
    public fun test_enable_attester_revert_zero_address() {
      let mut scenario = test_scenario::begin(@0x0);
      ...
      // Test: Revert if the new attester is the zero address
      scenario.next_tx(attester_manager);
      {
        enable_attester(@0x0, &mut state, scenario.ctx());
      };
