...
module message_transmitter_v2::role_management {
    // === Imports ===
    use sui::event::emit;
    ...
    /// Called by the owner to update the `rescuer` role.
    ...
    #[test]
    #[expected_failure(abort_code = EInvalidRoleAddress)]
    public fun test_update_attester_manager_revert_zero_address() {
        let (mut scenario, mut state) = setup();

        // Test: Revert if the new attester_manager is the zero address
        scenario.next_tx(OWNER);
        {
          update_attester_manager(@0x0, &mut state, scenario.ctx());
        };
