...
/// Module: initialize
module stablecoin_handler::initialize {
  // === Imports ===
  use sui_extensions::upgrade_service;
  use stablecoin_handler::state::{Self};
  ...
  /// init (automatically called at deployment) transfers an InitCap to the
  /// sender so they can call init_state to initialize state. Also creates and
  /// shares the wrapped Upgrade Service.
  ...
  /// Initializes and shares the State object with the owner set to the tx
  /// sender. Requires (and destroys) an InitCap object which can only be
  /// created in init, so this can only be called once.
  public fun init_state(init_cap: InitCap, ctx: &mut TxContext) {
    let state = state::new(ctx.sender(), ctx);
    state.share_state();
  ...
  #[test_only] use stablecoin_handler::state::{State};
  ...
  public fun test_init_successful() {
    ...
    let created_init_cap = scenario.take_from_address_by_id<InitCap>(
        @0x0, created_init_cap_id
    );
  ...
  fun test_init_state_successful() {
    ...
    // Test: After init_state is called, shared State was shared and InitCap was deleted.
    scenario.next_tx(@0x5);
    {
      init_state(init_cap, scenario.ctx());
