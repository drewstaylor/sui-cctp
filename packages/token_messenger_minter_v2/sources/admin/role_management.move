...
module token_messenger_minter_v2::role_management {
    // === Imports ===
    use sui::event::emit;
    ...
    const EInvalidRoleAddress: u64 = 1;
    ...
    /// Owner-only. Version + owner gate; the event and "same as old"
    /// check are enforced inside `cctp_extensions::rescuable`.
    entry fun update_rescuer(new_rescuer: address, state: &mut State, ctx: &TxContext) {
      assert_object_version_is_compatible_with_package(state.compatible_versions());
      state.roles().owner_role().assert_sender_is_active_role(ctx);
      state.rescuable_mut().update_rescuer(new_rescuer);
    }
    ...
    fun validate_role_transfer(new_address: address, current_address: address, state: &State, ctx: &TxContext) {
      state.roles().owner_role().assert_sender_is_active_role(ctx);
      assert!(new_address != @0x0, EInvalidRoleAddress);
    ...
    public fun test_update_pauser_revert_zero_address() {
        let (mut scenario, mut state) = setup();

        // Test: Revert if the new pauser is the zero address
        scenario.next_tx(OWNER);
        {
          update_pauser(@0x0, &mut state, scenario.ctx());
        };
    ...
    #[test]
    #[expected_failure(abort_code = EInvalidRoleAddress)]
    public fun test_update_token_controller_revert_zero_address() {
        let (mut scenario, mut state) = setup();

        // Test: Revert if the new token_controller is the zero address
        scenario.next_tx(OWNER);
        {
          update_token_controller(@0x0, &mut state, scenario.ctx());
        };
    ...
    public fun test_update_token_controller_revert_incompatible_version() {
        ...
        scenario.next_tx(OWNER);
        {
          update_token_controller(new_controller, &mut state, scenario.ctx());
          assert!(state.roles().token_controller() == OWNER);
        };
    ...
    #[test]
    public fun test_update_rescuer_successful() {
        let (mut scenario, mut state) = setup();
        let new_rescuer = @0x2;

        scenario.next_tx(OWNER);
        {
          update_rescuer(new_rescuer, &mut state, scenario.ctx());
          assert!(state.rescuable().rescuer() == new_rescuer);
          // Event emitted by `cctp_extensions::rescuable::update_rescuer`.
          assert!(num_events() == 1);
        };
    ...
    #[test]
    #[expected_failure(abort_code = two_step_role::ESenderNotActiveRole)]
    public fun test_update_rescuer_revert_not_owner() {
        let (mut scenario, mut state) = setup();
        let (non_owner, new_rescuer) = (@0x2, @0x3);
    ...
    #[test]
    #[expected_failure(abort_code = rescuable::EInvalidRescuer)]
    public fun test_update_rescuer_revert_zero_address() {
        let (mut scenario, mut state) = setup();

        // Test: Revert if the new rescuer is the zero address
        scenario.next_tx(OWNER);
        {
          update_rescuer(@0x0, &mut state, scenario.ctx());
        };
    ...
    #[test]
    #[expected_failure(abort_code = rescuable::ENewRescuerSameAsOld)]
    public fun test_update_rescuer_revert_already_set() {
        let (mut scenario, mut state) = setup();

        scenario.next_tx(OWNER);
        {
          update_rescuer(state.rescuable().rescuer(), &mut state, scenario.ctx());
        };
    ...
    #[test]
    #[expected_failure(abort_code = version_control::EIncompatibleVersion)]
    public fun test_update_rescuer_revert_incompatible_version() {
        let (mut scenario, mut state) = setup();
        let new_rescuer = @0x2;
        state.add_compatible_version(5);
        state.remove_compatible_version(version_control::current_version());
