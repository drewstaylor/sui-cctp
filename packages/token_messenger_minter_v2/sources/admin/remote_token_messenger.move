...
/// Owner-managed registry of remote token messengers per remote
/// domain (one-to-one).
module token_messenger_minter_v2::remote_token_messenger {
      // === Imports ===
    use sui::event::emit;
    ...
    /// Owner-only. Links `remote_domain -> remote_token_messenger`
    /// (32-byte hex address of the messenger on the remote chain).
    ...
    /// Owner-only. Unlinks the messenger registered for
    /// `remote_domain`, freeing the domain for a subsequent
