...
module token_messenger_minter_v2::version_control {
    use sui::vec_set::VecSet;

    /// The current version of the CCTP package. Used for `compatible_versions`
    /// gating during upgrades and migrations (see `migration.move`).
    ///
    /// NOTE: This is CCTP's own version counter and is distinct from Sui's
    /// native package version. Our `VERSION` is 1-indexed (starts at 1), whereas
    /// Sui's native version is 0-indexed: `UpgradeCap.version` (also surfaced via
    /// `sui_getObject`) is documented as "the number of upgrades that have been
    /// applied successively to the original package. Initially 0."
    const VERSION: u64 = 1;
...
#[test_only]
module token_messenger_minter_v2::version_control_tests {
    use sui::vec_set::{Self, VecSet};
    use token_messenger_minter_v2::version_control;
