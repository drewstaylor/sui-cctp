...
module token_messenger_minter_v2::initialize {
  // === Imports ===
  use token_messenger_minter_v2::state::{Self};
  ...
  /// Auto-called at deployment. Transfers an `InitCap` to the deployer
  /// (to call `init_state` next) and shares the wrapped Upgrade Service.
  ...
  /// One-shot: build + share the `State`, destroy the `InitCap`. All
  /// roles default to `ctx.sender()`; `paused = false`.
  ...
  #[test_only] use token_messenger_minter_v2::state::{State};
