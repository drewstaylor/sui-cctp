...
The examples exercise the V1 bridge against the EVM chain, so publish V1 with EVM linking (`yarn deploy-local:v1`, with the Anvil network running) following the steps above.
...
The V2 example scripts are Sui-only (no Anvil needed) — they synthesize the counterpart EVM message in TypeScript. Publish V2 first (`yarn deploy-local:v2`), then:
